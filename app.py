...
if prices.empty:
    st.error("Could not fetch prices; check symbols or try again.")
    st.stop()
...
with tab_dash:
    ...
    with c1: 
        ...
        st.metric("Portfolio Value", f"{float(values.sum()):,.2f} {base_ccy}")
        st.markdown('</div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="d-card">', unsafe_allow_html=True)
        st.metric("Risk Score (0–100)", score)
        st.pyplot(visuals.risk_gauge(score), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with c3:
        st.markdown('<div class="d-card">', unsafe_allow_html=True)
    ...
    with c4:
        st.markdown('<div class="d-card">', unsafe_allow_html=True)
        st.metric("Tracking Error (ann.)", f"{te:.2f}%")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with a:
        ...
        st.markdown('<div class="d-card">', unsafe_allow_html=True)
        st.pyplot(visuals.donut_series(w_sector, "Exposure by Sector"), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with b:
        corr = compute_correlation(rets)
        st.markdown('<div class="d-card">', unsafe_allow_html=True)
        st.pyplot(visuals.heatmap_corr(corr, "Correlation Heatmap"), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with p1:
        ...
        st.markdown('<div class="d-card">', unsafe_allow_html=True)
        st.pyplot(visuals.line_series(perf, "Performance vs Benchmark"), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with p2:
        tbl = positions.copy()
        last_conv = []
        ...
        if "avg_price" in tbl.columns and tbl["avg_price"].notna().any():
            ...
            for s,avg in zip(positions["symbol"], positions.get("avg_price", [])):
                ...
                ap.append(avgp)
            tbl["avg_price_base"] = ap
            tbl["pnl_%"] = ((tbl["last_price_base"]-tbl["avg_price_base"])/tbl["avg_price_base"]*100).round(2)
        st.markdown('<div class="d-card">', unsafe_allow_html=True)
...
with tab_factors:
    ...
    if not factor_prices.empty:
        X = factor_prices.pct_change().dropna()
        y = port_r.reindex(X.index).dropna()
        X = X.reindex(y.index)
        if len(X) >= 25 and y.std(ddof=0)>0:
            ...
            st.info("Not enough data to compute betas.")
    else:
        st.info("Could not load factor data right now.")
    st.markdown('</div>', unsafe_allow_html=True)
...
with tab_reb:
    ...
    st.dataframe(contrib_sector.sort_values(ascending=False).to_frame("risk_contribution"), use_container_width=True)

    suggestions = []
    if w.max()>0.30: suggestions.append("Trim the largest position below 30% to reduce concentration risk.")
    if w_sector.max()>0.50: suggestions.append("Diversify: dominant sector >50% of portfolio.")
    if port_r.std()>bench_r.std()*1.3: suggestions.append("Volatility above benchmark: consider adding low-beta names or cash.")
    if suggestions:
...
with tab_report:
    ...
    try:
        ...
        st.info("PDF generation requires reportlab. If it failed to install, remove it from requirements or use the Cloud build.")
    st.markdown('</div>', unsafe_allow_html=True)
