...
def donut_series(series: pd.Series, title: str = ""):
    ...
    wedges, _ = ax.pie(vals, startangle=140, colors=colors[:len(vals)])
...
def line_series(df: pd.DataFrame, title: str = ""):
    ...
    _dark_axes(ax)
    ax.legend(facecolor=PANEL, edgecolor="#1a1f29", labelcolor=TEXT)
    ax.set_title(title, color=TEXT, fontsize=11)
    return fig
...
def risk_gauge(score: float):
    ...
    # background arc
    ax.bar(angles, values*1, width=np.pi/100, bottom=0.0, color="#1a1f29")
    # pointer
